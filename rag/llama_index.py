...
class LLaMAIndexRAG(RAGInterface):
    ...
    def generate_group_response(self, query_info):
        """
        Generate an NPC response for a group chat, incorporating
        conversation history into the prompt.

        query_info must contain an additional key:
            'conversation_history': str  — formatted history from SessionManager
        """
        personality_type = query_info['personality']
        personality_prompt = get_personality_prompt(personality_type)
        is_rag = query_info['is_rag']
        conversation_history = query_info.get('conversation_history', '')
        ...
        if conversation_history:
            history_block = (
                "\n# 群組對話紀錄 \n"
                "以下是目前群組聊天室中的對話紀錄，請參考這些對話來理解上下文：\n"
                "---------------------\n"
        ...
        # Timing
        total_start_time = time.time()
        logger.info(f"Start of Group Chat Processing " + "=" * 10)
        ...
        if is_rag:
            ...
            logger.info(f"Synthesized Group Prompt:\n{synthesized_prompt}")
            ...
            try:
                ...
                print(f"OpenAI API error: {e}")
                return {"response": "抱歉，目前無法產生回應。", "metadata": {}}
        ...
        total_end_time = time.time()
        logger.info(f"Total time: {total_end_time - total_start_time:.2f} seconds")
        logger.info(f"End of Group Chat Processing " + "=" * 10 + "\n")
